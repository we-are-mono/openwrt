...
mkdir -p "$T/scripts" "$T/package/mono/mono-update-check/files" "$T/releases/mono-v25.12.5-r7"
...
"$USIGN" -G -s "$T/fleet.sec" -p "$T/package/mono/mono-update-check/files/mono-release.pub" -c fleet >/dev/null
"$USIGN" -G -s "$T/rot.sec"   -p "$T/package/mono/mono-update-check/files/mono-rotation.pub" -c rot >/dev/null
